...
int main(void) 
{
  ...
  for (kboucle = 1; kboucle < 31; kboucle++) {
    ...
    A = (MKL_Complex8 *)aligned_alloc(AVX512_ALIGN, M * K * sizeof(MKL_Complex8));
    B = (MKL_Complex8 *)aligned_alloc(AVX512_ALIGN, K * N * sizeof(MKL_Complex8));
    ...
    for (iboucle = 0; iboucle < K; iboucle++) {
      for (jboucle = 0; jboucle < N; jboucle++) {
        B[(iboucle * N) + jboucle].real = *(mat_real + (jboucle * K) + iboucle);

        B[(iboucle * N) + jboucle].imag =
            -1.0f * (*(mat_imag + (jboucle * K) + iboucle));
      }
    }
